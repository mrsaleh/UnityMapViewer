...
public partial class MapControl : INotifyPropertyChanged
{
    ...
    private void MapDataChanged(object sender, DataChangedEventArgs e)
    {
        RunOnUIThread(() =>
        {
            try
            {
                if (e == null)
                {
                    Debug.LogWarning("Unexpected error: DataChangedEventArgs can not be null");
                }
                else if (e.Cancelled)
                {
                    Debug.LogWarning("Fetching data was cancelled" + e.Error);
                }
                else if (e.Error is WebException)
                {
                    Debug.LogWarning("A WebException occurred. Do you have internet?" + e.Error);
                }
                else if (e.Error != null)
                {
                    Debug.LogWarning("An error occurred while fetching data" + e.Error);
                }
                else // no problems
                {
                    RefreshGraphics();
            ...
            {
                Debug.LogWarning($"Unexpected exception in {nameof(MapDataChanged)}" + exception);
            }
        });
    }
    ...
    private bool WidgetTouched(IWidget widget, Point screenPosition)
    {
        ...
        {
            Application.OpenURL(hyperlink.Url);
